 ...
 * This allows for easy navigation between items in a sequence.
 *
 * @template T - The type of elements in the series.
 * @param items - An array of items to form the series.
 * @param toKey - A function to generate unique keys for items. Defaults to string conversion.
 * @returns An object containing utility functions for working with the ordered series.
 *
 * @example
 * const weekdays = series(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]);
 *
 * console.log(weekdays.first()); // "Monday"
 * console.log(weekdays.last()); // "Friday"
 * console.log(weekdays.next("Wednesday")); // "Thursday"
 * console.log(weekdays.previous("Monday", "Sunday")); // "Sunday" (fallback default)
 * console.log(weekdays.spin("Wednesday", 3)); // "Monday"
 * console.log(weekdays.spin("Wednesday", -3)); // "Friday"
 */
